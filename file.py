...
def AddNote(filePath):
    ...
    mySet = set(get_my_list(filePath))
...
def ExportNotes(filePath):
    ...
    myList = get_my_list(filePath)                            # данные для экспорта в файл
    ...
    create_file(exportFilePath)                                      # создание файла - на всякий - необязательно
...
def ImportNotes(filePath):
    ...
    if (not os.path.exists(importFilePath)):
        ...
        tmp = "Error"
        return report, tmp
    listFromImport = get_my_list(importFilePath)
    with open(filePath, "a+", encoding="utf-8") as myFile:
...
def main() -> None:
    ...
    filePath = get_file_path(fileName)            # формирование пути для блокнота (рядом с Note.py)
    create_file(filePath)                        # на всякий случай во избежание проблем - необязательно
